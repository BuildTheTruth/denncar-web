...
export const getDocsFromCollection = async <T extends DocumentData>(path: string) => {
...
export const getDocFromCollection = async <T extends DocumentData>(path: string, id: string) => {
  const ref = doc(db, path, id).withConverter(firestoreDataConverter<T>())
  const snapshot = await getDoc(ref)
  return snapshot.data() ?? null
}

export const addDocToCollection = <T extends DocumentData>(path: string, data: T) =>
 ...
 */
export const updateDocInCollection = <T extends DocumentData>(
  path: string,
  id: string,
  { id: _id, createdAt: _createdAt, ...data }: T
) => updateDoc(doc(db, path, id), { ...data, updatedAt: serverTimestamp() })

export const deleteDocFromCollection = (path: string, id: string) => deleteDoc(doc(db, path, id))
